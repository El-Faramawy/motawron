                        ...
                        <td>
                            <div class="d-flex align-items-center">
								...
								<span class="symbol-label bg-light">
									<img class="h-100" onclick="window.open(this.src)" style='cursor: pointer' src={{asset($client->photo)}}  alt="">
								</span>
                                </div>
                                <a class="text-dark fw-bolder mb-1 fs-5">{{$client->name}}</a>
                            </div>
                        </td>
                        <td><span class="text-gray-500">{{$client->phone_code}}</span>{{$client->phone}}</td>
                        <td class="">
                            <button data-bs-toggle='modal' data-bs-target="#delete_car" class="btn btn-icon btn-bg-light btn-danger btn-sm me-1" data-toggle="modal" style="border-radius: 50% !important"
                                    data-id="{{$client->id}}" data-car_name="{{$client->name}}">
                            <span class="svg-icon svg-icon-3"><span class="svg-icon svg-icon-3"><i class="fa fa-trash"></i></span></span>
                            </button>
                        </td>
                    </tr>
                @endforeach
    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body scroll-y mx-3 mx-xl-15 my-3">
                <!--begin::Form-->
                <form id="kt_modal_new_card_form" class="form" action="{{route('delete.user')}}" method="post">
                    {{csrf_field()}}
